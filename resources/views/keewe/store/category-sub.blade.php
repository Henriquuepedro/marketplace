            ...
            <div class="col-md-3">
                <div class="subcategory-sidebar">
                    <ul class="list-unstyled level-1">
                        <li>
                            <a href="{{ url('/categoria/' . $main_category->slug) }}" class="h2">{{ $main_category->name }}</a>
                            <ul class="list-unstyled level-2">
                                <li>
                                    <a href="{{ url('/categoria/' . $category_lvl1->slug) }}" class="h3 {{ (('categoria/' . $category_lvl1->slug) == request()->path() ? 'active' : '') }}">
                                        {{ $category_lvl1->name }}
                                    </a>
                                    <ul class="list-unstyled level-3">
                                        @foreach( $category_lvl1->children as $subcategory )
                                            <li>
                                                <a href="{{ url('/categoria/' . $subcategory->slug) }}" class="{{ (('categoria/' . $subcategory->slug) == request()->path() ? 'active' : '') }}">
                                                    {{ $subcategory->name }}
                                                </a>
                                            </li>
                                        @endforeach
